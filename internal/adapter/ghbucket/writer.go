...
// PutFile implements [pubscoop.RepositoryWriter].
func (c *Client) PutFile(
	ctx context.Context,
	repository pubscoop.Repository,
	branch pubscoop.BranchName,
	path pubscoop.FilePath,
	previous pubscoop.BlobSHA,
	content []byte,
	message string,
) error {
	if err := c.requireReady(ctx); err != nil {
	...
	if err != nil {
		return classify(err, "publication manifest commit")
	}

	return nil
}

// CreatePullRequest implements [pubscoop.RepositoryWriter].
func (c *Client) CreatePullRequest(
	ctx context.Context,
	repository pubscoop.Repository,
	input pubscoop.PullRequestInput,
) (string, error) {
	if err := c.requireReady(ctx); err != nil {
		return "", err
	}
