...
// TestPutFileUpdatesOnlyExpectedCask proves an existing cask is replaced on
// the publication branch with the observed base blob SHA.
func TestPutFileUpdatesOnlyExpectedCask(t *testing.T) {
	t.Parallel()
	...
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		var payload struct {
			...
			// Content is the API-encoded cask body.
		...
		assert.Equal(t, "chore(cask): update release-cli to 1.2.3", payload.Message)
