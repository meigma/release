...
## 2026-08-21 23:59 — Cross-org adoption PR
Documentation: Two technical-writer passes replaced the README plus 17 documents with the approved Diátaxis set: one concise README, one tutorial, six how-to/operator guides, two references, and one architecture/trust explanation. The copyable Go example now includes every supported publisher, one immutable revision placeholder, and disabled-by-default publication controls. The Nix consumer example now pins `v0.1.16`.
