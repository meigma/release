...
- Every package, including every `mocks` package and `package main`, gets `doc.go` (D4). Every production function, type, method, and struct field—exported or not—gets Godoc; public command/domain boundaries get fuller contract comments and useful examples (D1, D2). **Standing D1 interpretation for tests:** `TestXxx` functions and table-struct fields are exempt, matching the repo's pre-existing tests, because the rule exists for IDE navigation and those names are self-describing; non-obvious **test helpers** are documented. Keep source files below 1,000 lines (R2).
- Keep `release-cli` noninteractive and pipeline-safe, per command: `version` prints its data to stdout (human line by default, JSON envelope under `--json`) because that data *is* its output; side-effecting commands such as `stage` print nothing to stdout on success without `--json`, send diagnostics and warnings to stderr, and emit exactly one envelope on stdout under `--json` (including failures, with `ok:false`). The `--json` envelope is the only stable stdout contract and is exactly `{"schema":"release.dev/result/v1","command":...,"ok":...,"result":...}`. Exit codes are only `0`, `1`, and `2`. There is no `phase`, `mutations`, generic safe-rerun claim, or exit `3`.
- Resolve flags and `RELEASE_*` environment variables in the command layer with flag-over-env precedence (`Flag.Changed`) and **no config file**. Viper was removed in PR 1 under L1: six calls for three environment variables did not justify its dependency tree in a supply-chain tool, and `release.toml` is trigger-gated. Keep a small injected config seam so a reader can be reintroduced locally when that trigger fires. Unknown profiles and invalid flags/configuration are usage/config failures (exit `2`); contract, verification, and fail-closed remote-state failures are exit `1`.
...
6. **Caller-ceiling audit.** Whenever a PR changes a *callee* job's `permissions`, verify every calling job grants at least that much — a called workflow can never exceed its caller's ceiling, and the whole run is rejected at startup with no jobs and no API-visible annotation. PR 2 shipped `attestations: read` on `go-oci-build.yml`'s job without raising `release.yml`'s `oci-image` ceiling, and it was unreachable by PR CI because `release.yml` only runs on tags. Audit both this repo's `release.yml` and the copyable consumer caller in `examples/go-release/`, mechanically comparing each callee job's maximum requested permission against its caller's grant.
7. **Parent gate before opening the PR.** The parent runs `mise exec -- moon run root:check` (format, lint, build, test, protocol stamp), exercises the changed behavior directly (not just its tests), and confirms the PR's "Must NOT contain" list is honored.
8. **Open the PR for human acceptance.** Conventional Commit title from the spine table, body summarizing scope, verification evidence, and any recorded follow-ups. The parent never merges: a human accepts.
9. **Journal.** Record the PR's outcome, review/conformance findings, and follow-ups in the bound session's `NOTES.md` before yielding.
...
- Create `internal/profile/goprof/testdata/goreleaser-2.17.1-artifacts.json` (captured multi-target, not `--single-target`, so `SelectBinaries` is exercised against real records) — verbatim parser fixture captured from pinned GoReleaser; document its refresh command in the fixture test, not a new package.
...
- Modify `.goreleaser.yaml` — `project_name`, build/archive IDs, `main`, and binary name become `release-cli`; retain platforms, archive/SBOM/checksum/signing, and `release.disable: true`. **No protocol ldflag:** per the Revision 3 contract the protocol is a Go source constant (`cli.Protocol`) guarded by `scripts/check-protocol-stamp.sh`, so only `main.version` and `main.commit` are injected.
