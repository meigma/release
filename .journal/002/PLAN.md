...
**Fallback:** in each reusable job, explicitly check out the callee repository at `github.job_workflow_sha` to a dedicated path and invoke its local composite path. Derive repository identity from `github.job_workflow_ref`; do not hardcode `meigma/release`. This keeps the same action, CLI, one consumer pin, and all ports; it only adds residual checkout plumbing.
